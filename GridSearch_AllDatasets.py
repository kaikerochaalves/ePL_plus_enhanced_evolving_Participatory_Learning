...
result = pd.DataFrame(columns = ['Alpha', 'Beta', 'Lambda', 'omega', 'sigma', 'e_Utility', 'pi', 'RMSE', 'NDEI', 'MAE', 'Rules'])
for alpha in l_alpha:
    for beta in l_beta:
        for lambda1 in l_lambda1:
            for omega in l_omega:
                for sigma in l_sigma:
                    for e_Utility in l_e_Utility:
                        for pi in l_pi:
                            
                            print(f"Alpha = {alpha}, Beta = {beta}, Lambda = {lambda1}, omega = {omega}, sigma = {sigma}, e_Utility = {e_Utility}, pi = {pi}")
                            ...
                            result = result.append({'Alpha': alpha, 'Beta': beta, 'Lambda': lambda1, 'omega': omega, 'sigma': sigma, 'e_Utility': e_Utility, 'pi': pi, 'RMSE': RMSE, 'NDEI': NDEI, 'MAE': MAE, 'Rules': Rules}, ignore_index=True)
